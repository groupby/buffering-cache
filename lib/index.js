...
const BufferingCache = function (remoteCacheSpec, localCacheSpec, options) {
    ...
    if (!_.isNil(options)) {
        if (!_.isNil(options.cacheName)) {
            if (!_.isString(options.cacheName) || options.cacheName.length === 0 || options.cacheName.length > 25) {
                throw new Error('options.cacheName, if provided, must be a string between 1 and 25');
            } else {
    ...
    // Add default cacheName to simplify hooks
    if (_.isNil(cacheName)) {
        cacheName = 'unspecified';
    }
            ...
            return self.localCache.get(key)
                .then((localValue) => {
                    if (typeof localValue !== 'undefined') {
                        ...
                        onInMemoryCacheHit(cacheName);

                        return (postCallMutator(localValue, CACHE_LEVEL.LOCAL));
                    }

                    onInMemoryCacheMiss(cacheName);
