...
const BufferingCache = function (remoteCacheSpec, localCacheSpec) {
  ...
  if (remoteCacheSpec.ttl < localCacheSpec.ttl) {
  ...
  const updateLocalCache = (key, value, remainingRemoteTtl) => {
    const localCacheTtl = (remainingRemoteTtl && remainingRemoteTtl < localCacheSpec.ttl) ? remainingRemoteTtl : localCacheSpec.ttl;

    return localCache.setpx(key, value, localCacheTtl);
  };
  ...
  self.__refreshBuffer = (key, cachedValue, action, that, args) => {
    ...
    return remoteCache.ttl(key).then((ttl) => {
      ...
      log.debug(`TTL: ${ttl} BufferTTL: ${remoteCacheSpec.bufferTtl} Remaining: ${minTimeRemaining}`);
      ...
      if (!ttl || ttl < minTimeRemaining) {
        ...
        return Promise.resolve(action.apply(that, args))
          .then((response) => remoteCache.setpx(key, response)
            .then(() => updateLocalCache(key, response, ttl)));
      } else {
