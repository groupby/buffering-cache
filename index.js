...
module.exports = function ({
    ...
    if (_.isNil(ttlMsec) || !_.isNumber(ttlMsec)) {
        throw new Error('ttlMsec must be a number and must be provided');
    }

    if (!_.isNumber(bufferTtlMsec) || bufferTtlMsec < 0 || bufferTtlMsec > ttlMsec) {
        throw new Error('bufferTtlMsec, if provided, must be a number greater than 0 and less than or equal to ttlMsec');
    }

    if (!_.isNumber(localCacheSize) || localCacheSize < 0) {
        throw new Error('localCacheSize, if provided, must be a number greater than or equal to 0');
    }

    if (!_.isNumber(localTtlMsec) || localTtlMsec <= 0 || localTtlMsec > bufferTtlMsec) {
